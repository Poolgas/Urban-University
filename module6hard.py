...
class Figure():
    ...
    def __init__(self, color=(0, 0, 0), *sides):
        ...
        self.filled: bool
        if len(sides) == 1:
            self.__sides = [abs(sides[0]) for _ in range(self.sides_count)]
        elif len(sides) != self.sides_count :
            self.__sides = [1 for _ in range(self.sides_count)]
        else:
            self.__sides = [abs(i) for i in sides]
...
print(f'Проверка создания треугольника с тремя отрицательными сторонами: {Triangle((200, 200, 100), -10, -6, -14).get_sides()}')
