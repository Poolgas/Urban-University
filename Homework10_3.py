...
По итогу вы получите скрипт блокирующий поток до баланса равному 500 и больше или блокирующий, когда происходит попытка снятия при недостаточном балансе.
...
class Bank:
    ...
    def deposit(self):
        for i in range(100):
            ...
            if self.balance >= 500 and self.lock.locked():
                self.lock.release()
            '''Изменение переменной для принудительной остановки цикла'''
            if i == 99:
                self.end = True
                if self.lock.locked():
                    self.lock.release()
    ...
    def take(self):
        for i in range(100):
            ...
            if take > self.balance and self.end == False:
                ...
                '''Остановка цикла если баланс кончился и пополнение баланса остановилось'''
            elif self.end == True:
                return
